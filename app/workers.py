...
from app.database import engine, db_write_lock
from app.logic import call_llm, analyze_feedback_hybrid, sanitize_text, map_topics_to_department
from app.models import Feedback, AnalysisSource
...
async def process_csv_worker(csv_content: str):
    ...
    with Session(engine) as session:
        ...
        for row in csv_reader:
            ...
            # Dedup check
            existing = session.exec(select(Feedback).where(Feedback.content_hash == text_hash)).first()
...
async def run_periodic_reconciliation():
    ...
    while True:
        try:
            ...
            with Session(engine) as session:
                statement = select(Feedback).where(Feedback.source == AnalysisSource.FALLBACK).limit(10)
                items = session.exec(statement).all()
                
                for item in items:
                    await reconcile_data_worker(str(item.id))
                    # Small delay between items to prevent hogging the lock
